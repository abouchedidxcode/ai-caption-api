...
**Narrator's Caption:** [your caption here]`,
};
...
async function processWithOpenAI(imageData, mimeType) {
  ...
  try {
    ...
    const dogThoughtMatch = rawContent.match(/\*\*Dog's Thought:\*\*\s*(.+?)(?=\*\*Narrator's Caption:\*\*|$)/s);
    const narratorCaptionMatch = rawContent.match(/\*\*Narrator's Caption:\*\*\s*(.+?)(?=\*\*|$)/s);
    
    const dogThought = dogThoughtMatch ? dogThoughtMatch[1].trim() : null;
    const narratorCaption = narratorCaptionMatch ? narratorCaptionMatch[1].trim() : null;
    
    // Fallback: if parsing fails, return the raw content as caption
    if (!dogThought || !narratorCaption) {
      return {
        caption: rawContent,
        dogThought: null,
